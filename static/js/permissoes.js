...
let usuarios = []
let requisicoes = []
let usuarioLogado = null // Store logged user info
...
async function carregarSessao() {
  try {
    ...
    document.getElementById("usuarioNome").textContent = usuarioLogado.usuario_nome
...
async function atualizarContadorRequisicoes() {
  try {
    const response = await fetch("/api/requisicoes/count")
    const data = await response.json()
    document.getElementById("badgeRequisicoes").textContent = data.count
    document.getElementById("badgeTabRequisicoes").textContent = data.count
  } catch (error) {
...
function renderizarUsuarios() {
  ...
  usuarios.forEach((usuario) => {
    ...
    const isCurrentUser = usuarioLogado && usuario.idUsu === usuarioLogado.usuario_id
    const deleteButton = isCurrentUser
      ? '<button class="action-btn delete" disabled title="Você não pode excluir seu próprio usuário">Remover</button>'
      : `<button class="action-btn delete" onclick="removerUsuario(${usuario.idUsu})">Remover</button>`

    tr.innerHTML = `
            <td>${usuario.nome}${isCurrentUser ? " (eu)" : ""}</td>
            <td>${usuario.email}</td>
...
async function adicionarUsuario() {
  ...
  const idFuncao = Number.parseInt(document.getElementById("novaFuncao").value)
...
async function salvarEdicaoUsuario() {
  const idUsu = Number.parseInt(document.getElementById("editarIdUsu").value)
  const nome = document.getElementById("editarNome").value
  const email = document.getElementById("editarEmail").value
  const idFuncao = Number.parseInt(document.getElementById("editarFuncao").value)
...
async function aceitarRequisicao(idReq) {
  const idFuncao = Number.parseInt(document.getElementById("funcaoRequisicao").value)
  ...
  try {
    ...
    if (response.ok) {
      ...
      carregarRequisicoes()
      carregarUsuarios()
      atualizarContadorRequisicoes() // Update counter after accepting
    } else {
      const error = await response.json()
      alert("Erro: " + error.erro)
    }
...
async function recusarRequisicao(idReq) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      fecharModalRequisicao()
      carregarRequisicoes()
      atualizarContadorRequisicoes() // Update counter after refusing
    } else {
      const error = await response.json()
      alert("Erro: " + error.erro)
    }
